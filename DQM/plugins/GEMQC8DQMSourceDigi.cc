...
class GEMQC8DQMSourceDigi: public DQMEDAnalyzer
{
public:
  GEMQC8DQMSourceDigi(const edm::ParameterSet& cfg);
  ~GEMQC8DQMSourceDigi() override {};
...
int GEMQC8DQMSourceDigi::findVFAT(float min_, float max_, float x_, int roll_) {
  float step = max_/3;
...
const GEMGeometry* GEMQC8DQMSourceDigi::initGeometry(edm::EventSetup const & iSetup) {
...
GEMQC8DQMSourceDigi::GEMQC8DQMSourceDigi(const edm::ParameterSet& cfg)
{
  tagDigi_ = consumes<GEMDigiCollection>(cfg.getParameter<edm::InputTag>("digisInputLabel")); 
}

void GEMQC8DQMSourceDigi::fillDescriptions(edm::ConfigurationDescriptions & descriptions)
{
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("digisInputLabel", edm::InputTag("muonGEMDigis", "")); 
  descriptions.add("GEMQC8DQMSourceDigi", desc);  
}

void GEMQC8DQMSourceDigi::bookHistograms(DQMStore::IBooker &ibooker, edm::Run const &, edm::EventSetup const & iSetup)
{
...
void GEMQC8DQMSourceDigi::analyze(edm::Event const& event, edm::EventSetup const& eventSetup)
{
...
DEFINE_FWK_MODULE(GEMQC8DQMSourceDigi);
