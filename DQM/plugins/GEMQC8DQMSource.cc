...
class GEMQC8DQMSource: public DQMEDAnalyzer
{
public:
  GEMQC8DQMSource(const edm::ParameterSet& cfg);
  ~GEMQC8DQMSource() override {};
...
int GEMQC8DQMSource::findVFAT(float min_, float max_, float x_, int roll_) {
...
const GEMGeometry* GEMQC8DQMSource::initGeometry(edm::EventSetup const & iSetup) {
...
GEMQC8DQMSource::GEMQC8DQMSource(const edm::ParameterSet& cfg)
{
  tagRecHit_ = consumes<GEMRecHitCollection>(cfg.getParameter<edm::InputTag>("recHitsInputLabel")); 
}

void GEMQC8DQMSource::fillDescriptions(edm::ConfigurationDescriptions & descriptions)
{
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("recHitsInputLabel", edm::InputTag("gemRecHits", "")); 
  descriptions.add("GEMQC8DQMSource", desc);  
}

void GEMQC8DQMSource::bookHistograms(DQMStore::IBooker &ibooker, edm::Run const &, edm::EventSetup const & iSetup)
{
...
void GEMQC8DQMSource::analyze(edm::Event const& event, edm::EventSetup const& eventSetup)
{
  //std::cout << "GEMQC8DQMSource::analyze irun=" << event.run() << " event=" << event.id().event() << std::endl;
  ...
  if (!gemRecHits.isValid()) {
    edm::LogError("GEMQC8DQMSource") << "GEM recHit is not valid.\n";
    return ;
  }  
  for (auto ch : gemChambers_){
    GEMDetId cId = ch.id();
...
DEFINE_FWK_MODULE(GEMQC8DQMSource);
